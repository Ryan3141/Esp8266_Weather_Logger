...
{
	return this->microseconds - t.microseconds < 0;
}
...
{
	Time now;
	//now.microseconds = (millis() + 0x1FE363C7FUL);
	now.microseconds = millis();
	now.microseconds *= 1000;
//	now.microseconds += micros();
	//now.microseconds = ((long long) millis()) * 1000 + micros();
	return now;
}
